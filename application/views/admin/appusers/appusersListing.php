  ...
  <div class="">
    <div class="page-title">
      ...
      <div class="title_right">
        <form name="search_form" id="search_form" action="<?php echo site_url($this->PANEL . '/' . $this->CONTROLLER . '/index/s/'); ?>" method="post">
          <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right top_search">
            <div class="input-group">
              <input type="text" class="form-control"  value="<?php echo $searchKey ?>" name="searchText"  placeholder="Search for..." style="border-radius: 0px;">
		...
		?>
        <select name="user_service_type" class="form-control">
            <?php
			foreach($srchUserTypeArr as $srchUserType){
				echo '<option value="'.$srchUserType['backend'].'"  '.($searchUserType == $srchUserType['backend'] ? 'selected' : '' ).'>'.$srchUserType['frontend'].'</option>';
			}
			?>
            </select>
            </div>
    ...
    <div class="row">
      ...
      <div style="float:left"><?php echo getMessage(); ?></div>
        <div style="float:right"> <a href="<?php echo site_url($this->PANEL . '/' . $this->CONTROLLER . '/' . $this->EDIT_CONTROLLER_NAME); ?>" type="submit" class="btn btn-primary">Add <?php echo $this->TITLE ?></a> <a href="javascript:$('#del_btn').trigger('click');" class="btn btn-danger">Delete Selected <?php echo $this->TITLE ?></a>
      ...
      <table class="table table-striped responsive-utilities jambo_table bulk_action">
        <thead>
          <tr class="headings">
            ...
            <th class="column-title">Sevice Type</th>
        ...
        <tbody>
          ...
          <tr class="even pointer" id="row_<?php echo $id ?>">
           <td class="a-center "><input type="checkbox" value="<?php echo $id ?>"   class="flat" name="deletes[]"></td>
             <!--<td class=""><?php //if($list['customer_photo']!=''){?>
            <img src="<?php //echo base_url().$list['customer_photo']; ?>" width="100" />
            <?php //}else{ echo 'No Image'; } ?>	
                
                </td>-->
            <td class=""><?php echo $list['user_name']  ?></td>
            <td class=""><?php echo $list['user_email']  ?></td>
            <td class="">
			<?php 
			if($list['user_service_type'] != 0){
				echo 'Service Seeker';
			}else{
